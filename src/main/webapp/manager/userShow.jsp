...
<body>
	<h2>用户信息</h2>
	<br>
	<form action="/news/servlet/UserServlet?type1=showPage" id="myform"
		method="post">
		<table align="center" class="tableDefault" style="width:500px;">
			...
			<c:forEach items="${requestScope.users}" var="user"
				varStatus="status">
				<c:if test="${user.name!='anonymous' }">
					<tr class="trDefault">
						<td>${user.userId}</td>
						<td>${user.type}</td>
						<td><c:if test="${user.name!='anonymous'}">
								<a
									href="/news/servlet/UserServlet?type1=showOne&oneUserName=${user.name}"
									target='frame'>
							</c:if> ${user.name} <c:if test="${user.name!='anonymous'}">
								</a>
							</c:if></td>
